...
Object.defineProperty(state, "transportPlan", {
  get() {
    if (!state.transportDays.length) {
      state.transportDays.push({ origin: "", originNote: "", originNoteExpanded: false, points: [], expanded: true, segmentTimes: {}, segmentCosts: {}, extraCosts: [], costSplitPeople: "1", goalCostExpanded: false, completed: false, orderKey: 0 });
...
function loadTransportDays() {
  const fallback = [{ origin: "", originNote: "", originNoteExpanded: false, points: [], expanded: true, completed: false, extraCosts: [], costSplitPeople: "1", goalCostExpanded: false, orderKey: 0 }];
  ...
  const normalized = days.map((day, idx) => {
    ...
    const costSplitPeople = normalizePointCostPeopleValue(day?.costSplitPeople || "1");
    const goalCostExpanded = Boolean(day?.goalCostExpanded);
    const orderKey = Number.isFinite(Number(day?.orderKey)) ? Number(day.orderKey) : idx;
    const next = { origin, originNote, originNoteExpanded, points, expanded, headerEstimate, segmentTimes, segmentCosts, completed, extraCosts, costSplitPeople, goalCostExpanded, orderKey };
...
function captureTransportUndoSnapshot() {
  const days = (state.transportDays || []).map((day, idx) => {
    ...
    const goalCostExpanded = Boolean(day?.goalCostExpanded);
    const orderKey = Number.isFinite(Number(day?.orderKey)) ? Number(day.orderKey) : idx;
    return { origin, originNote, originNoteExpanded, points, expanded, headerEstimate, segmentTimes, segmentCosts, completed, extraCosts, costSplitPeople, goalCostExpanded, orderKey };
  });
  return {
    transportDays: days,
...
function applyDayHeaderTimeSelectChange(dayHeaderTimeSelect) {
...
function applySegmentTimeSelectChange(segmentTimeSelect, { showStatus = false } = {}) {
  const { dayIndex, pointIndex } = parseDayAndIndex(segmentTimeSelect?.dataset?.segmentTimeSelect);
  if (!Number.isInteger(dayIndex) || !Number.isInteger(pointIndex)) return false;
  clearUndoTypingSession();
...
function ensurePointObject(point) {
  ...
  const hasAnyCostValue = costItems.some((item) => String(item?.amount || "").trim() !== "");
  const costExpanded =
    typeof point?.costExpanded === "boolean"
      ? point.costExpanded
      : hasAnyCostValue;
  return {
    name: point?.name || "",
...
function renderDestinationCostControls(dayIndex) {
  ...
  host.classList.toggle("hidden", !isGoalCostOpen);
  host.style.display = isGoalCostOpen ? "grid" : "none";
  host.innerHTML = `
    ${
      costItems
        .map((item, idx) => {
  ...
  if (toggleBtn) {
    toggleBtn.textContent = isGoalCostOpen ? "×" : "金額";
    toggleBtn.classList.toggle("note-toggle-active", isGoalCostOpen);
  }
}

function toggleDayGoalCostAccordion(dayIndex) {
  if (!Number.isInteger(dayIndex) || dayIndex < 0 || dayIndex >= state.transportDays.length) return;
  const day = state.transportDays[dayIndex];
  if (!day) return;
  normalizePlanPoints(day);
...
function resolveDayIndexFromGoalCostButton(button) {
  if (!button) return -1;
  ...
  const cards = ensureDayCardAccordionStructure();
  const byPosition = cards.findIndex((c) => c.contains(button));
  if (byPosition >= 0 && byPosition < state.transportDays.length) {
    return byPosition;
  }
  return -1;
...
function syncDayCardInputsFromState(dayIndex) {
  ...
  if (originNoteToggleBtn) {
    originNoteToggleBtn.textContent = day.originNoteExpanded ? "×" : "備考";
  ...
  if (destinationInput) {
    ...
    if (destinationNoteToggleBtn) {
      ...
      destinationNoteToggleBtn.textContent = isOpen ? "×" : "備考";
...
function ensureDayCardsMatchStateDays() {
  ...
  if (!Array.isArray(state.transportDays) || !state.transportDays.length) {
    state.transportDays = [{ origin: "", originNote: "", originNoteExpanded: false, points: [], expanded: true, headerEstimate: "", segmentTimes: {}, segmentCosts: {}, extraCosts: [], costSplitPeople: "1", goalCostExpanded: false, completed: false, orderKey: 0 }];
  }
...
function clearDaySectionByButton(button) {
  ...
  day.segmentCosts = {};
  day.extraCosts = [];
  day.costSplitPeople = "1";
  day.goalCostExpanded = false;
  day.completed = false;
...
function normalizeAllDays() {
  state.transportDays.forEach((day, idx) => {
    ...
    day.costSplitPeople = normalizePointCostPeopleValue(day.costSplitPeople || "1");
    day.goalCostExpanded = Boolean(day.goalCostExpanded);
  });
}
...
function getDaySplitPeople(day) {
  return normalizePointCostPeopleValue(day?.costSplitPeople || "1");
}
...
function renderDayCostSummary(dayIndex) {
  ...
  const splitPeople = Math.max(1, Number(getDaySplitPeople(day)) || 1);
      ...
      <li class="route-item route-waypoint-item ${isEmptyWaypoint ? "is-empty-waypoint" : ""}" draggable="true" data-day-index="${dayIndex}" data-index="${sourceIndex}">
        ...
        <div class="route-waypoint-body ${point.expanded ? "" : "hidden"}">
          <div class="route-history-tools route-waypoint-history-tools">
            ...
            <button type="button" class="ghost tiny ${point.costExpanded ? "note-toggle-active" : ""}" data-toggle-point-cost="${dayIndex}:${sourceIndex}">${point.costExpanded ? "×" : "金額"}</button>
            <button type="button" class="ghost tiny ${point.noteExpanded ? "note-toggle-active" : ""}" data-toggle-point-note="${dayIndex}:${sourceIndex}">${point.noteExpanded ? "×" : "備考"}</button>
          </div>
...
function undoLastTransportAction() {
  ...
  state.transportDays = snapshot.transportDays.map((day, idx) => {
    const normalized = {
      ...
      costSplitPeople: normalizePointCostPeopleValue(day?.costSplitPeople || "1"),
      goalCostExpanded: Boolean(day?.goalCostExpanded),
  ...
  if (!state.transportDays.length) {
    state.transportDays = [{ origin: "", originNote: "", originNoteExpanded: false, points: [], expanded: true, segmentTimes: {}, segmentCosts: {}, extraCosts: [], costSplitPeople: "1", goalCostExpanded: false, completed: false, orderKey: 0 }];
  }
...
document.addEventListener("change", async (e) => {
  ...
  if (dayHeaderTimeSelect) {
    applyDayHeaderTimeSelectChange(dayHeaderTimeSelect);
    return;
  }
...
document.addEventListener("focusin", (e) => {
  const dayScopedField = e.target.closest(
    "input[data-day-role='origin-input'], input[data-day-role='destination-input'], input[data-point-input], input[data-point-url], select[data-day-role='day-header-time-select'], input[data-point-cost-amount], select[data-point-cost-people], select[data-point-cost-type], input[data-extra-cost-amount], select[data-extra-cost-type], input[data-segment-cost-input], select[data-day-split-people]",
  );
...
routeEventsHost.addEventListener("input", (e) => {
  ...
  if (segmentTimeSelect) {
    applySegmentTimeSelectChange(segmentTimeSelect, { showStatus: false });
    return;
  }
...
routeEventsHost.addEventListener("change", (e) => {
  ...
  const segmentTimeSelect = e.target.closest("select[data-segment-time-select]");
  if (segmentTimeSelect) {
    applySegmentTimeSelectChange(segmentTimeSelect, { showStatus: true });
    return;
  }
  ...
  const daySplitPeopleSelect = e.target.closest("select[data-day-split-people]");
  if (daySplitPeopleSelect) {
    const dayIndex = Number(daySplitPeopleSelect.dataset.daySplitPeople);
    const day = state.transportDays[dayIndex];
    if (!day) return;
    setActiveDay(dayIndex);
    const normalizedPeople = normalizePointCostPeopleValue(daySplitPeopleSelect.value || "1");
    daySplitPeopleSelect.value = normalizedPeople;
    day.costSplitPeople = normalizedPeople;
    renderDayCostSummary(dayIndex);
    saveTransportState();
    return;
  }
